...
def simulate_play(winning_rate, total_winning_needed, winning_streak_bounce):
    total_count = 0
    star_count = 0
    ...
    while(star_count < total_winning_needed and total_count <= 350):
        ...
        if(r <= 100 * winning_rate):
            ...
            star_bounce = (winning_streak_bounce - int(star_count / 15)) if (winning_streak_bounce - int(star_count / 15)) >= 1 else 1
...
def show_figure(all_total_counts, total_winning_needed, winning_streak_bounce, winning_rate, file_id):
    ...
    ax0.set_title('--PDF-- \nStar needed: ' + str(total_winning_needed) +
                  '\nWinning streak bounce: ' + str(winning_streak_bounce) +
...
def main(total_winning_needed, winning_streak_bounce, winning_rate,file_id):
    all_total_counts = [[], [], []]
    COMPARE_NUM = 3
    UNCERTAINITY = 0.03
    total_winning_needed = int(total_winning_needed)
    winning_streak_bounce = int(winning_streak_bounce)
    ...
    for ii in range(test_time):        
        for jj in range(COMPARE_NUM):
            total_count = simulate_play(winning_rates[jj], total_winning_needed, winning_streak_bounce)
            all_total_counts[jj].append(total_count)

    show_figure(all_total_counts, total_winning_needed, winning_streak_bounce, winning_rate,file_id)
